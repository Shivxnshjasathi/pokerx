...
## GetMySessions
You can execute the `GetMySessions` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getMySessions(): QueryPromise<GetMySessionsData, undefined>;
...
```typescript
getMySessions(dc: DataConnect): QueryPromise<GetMySessionsData, undefined>;
...
The `data` property is an object of type `GetMySessionsData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getMySessions } from '@dataconnect/generated';
...
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getMySessions(dataConnect);

console.log(data.sessions);

// Or, you can use the `Promise` API.
getMySessions().then((response) => {
...
import { connectorConfig, getMySessionsRef } from '@dataconnect/generated';


// Call the `getMySessionsRef()` function to get a reference to the query.
const ref = getMySessionsRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getMySessionsRef(dataConnect);
...
## RecordSession
You can execute the `RecordSession` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
recordSession(vars: RecordSessionVariables): MutationPromise<RecordSessionData, RecordSessionVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
recordSession(dc: DataConnect, vars: RecordSessionVariables): MutationPromise<RecordSessionData, RecordSessionVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the recordSessionRef:
```typescript
const name = recordSessionRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `RecordSession` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `RecordSessionData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface RecordSessionData {
  session_insert: Session_Key;
}
```
### Using `RecordSession`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, recordSession, RecordSessionVariables } from '@dataconnect/generated';

// The `RecordSession` mutation requires an argument of type `RecordSessionVariables`:
...
// Call the `recordSession()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await recordSession(recordSessionVars);
// Variables can be defined inline as well.
const { data } = await recordSession({ sessionDate: ..., gameTypeId: ..., buyIn: ..., cashOut: ..., locationId: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await recordSession(dataConnect, recordSessionVars);

console.log(data.session_insert);

// Or, you can use the `Promise` API.
recordSession(recordSessionVars).then((response) => {
  const data = response.data;
  console.log(data.session_insert);
});
```

### Using `RecordSession`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, recordSessionRef, RecordSessionVariables } from '@dataconnect/generated';
...
// Call the `recordSessionRef()` function to get a reference to the mutation.
const ref = recordSessionRef(recordSessionVars);
// Variables can be defined inline as well.
const ref = recordSessionRef({ sessionDate: ..., gameTypeId: ..., buyIn: ..., cashOut: ..., locationId: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = recordSessionRef(dataConnect, recordSessionVars);
